...
create table inversment(
     inversment_id int auto_increment primary key,
     user_id int,
     type varchar(50),
     amount decimal(10,2) not null,
...
INSERT INTO Users (username, role, password) VALUES
 ...
 select users.user_id, (SUM(income.amount) - SUM(expenses.amount) - SUM(debts.amount)) / SUM(income.amount) * 100
     as score, NOW()
     FROM users
     LEFT JOIN income ON users.user_id = income.user_id
     LEFT JOIN expenses ON users.user_id = expenses.user_id
...
START TRANSACTION;
 ...
 INSERT INTO income(user_id,source,amount,date)
     VALUES(1,'side hustle',9000.00,'2023-10-11');
 SAVEPOINT S2;
 UPDATE savings SET amount=amount+9000.00
 ...
 SELECT * FROM DEBTS;
 SAVEPOINT S3;
 UPDATE SAVINGS SET amount = amount-1000.00
     WHERE user_id = 1 AND account_type = 'emergency fund';
SAVEPOINT S4;
 UPDATE debts SET amount = amount-1000.00
     WHERE debts_id = 1;
 COMMIT;
 ...
 SELECT * FROM savings;
 SAVEPOINT S5;
 UPDATE savings SET amount = amount-500.00
    WHERE user_id = 1 AND account_type = 'emergency fund';
 SAVEPOINT S6;
 UPDATE savings SET amount = amount+500.00
 ...
 SAVEPOINT S7;
 UPDATE savings SET amount = amount-2000.00
     WHERE user_id = 1 AND account_type = 'emergenct fund';
 SAVEPOINT S8;
 INSERT INTO investments(user_id,type,amount,return_rate,date)
     VALUES(1,'Mutual Fund',2000.00,6.00,'2023-10-03');
 COMMIT;
